...
void main()
{
    ...
    int p = 6, size = 2 * p * 32 / 8;
    uint32_t Z[12] = {0}, Sj[2 * p], S[2 * p], tempo[2 * p];
    uint8_t tmp[size], out[size], temp[size + 4];
    int X[2 * p * 32][1000];

    InvAtlantickRoundFunction(Z, p, 0);
    sha3_384(out, size, convert_array_32_to_8_bit(Z, 2 * p, tmp), 2 * p * 32 / 8);
    ...
    for (int s = 1; s <= round; s++)
    {
        ...
        for (int j = 0; j <= 2 * p * 32 - 1; j++)
        {
            ...
            for (int t = 0; t <= 999; t++)
            {
                ...
                sha3_384(out, size, convert_array_32_to_8_bit(Zn, 2 * p + 1, temp), (2 * p + 1) * 32 / 8);
