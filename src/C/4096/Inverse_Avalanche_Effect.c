/** Avalanche Effect Study of the Inverse Atlantick Permutation
...
{
    uint32_t C, L = 4, temp[L], tempB[p], *tempo;
    ////////////// MIXA STAR
...
void main()
{
    ...
    int p = 64, size = 2 * p * 32 / 8;
    uint32_t Z[128] = {0}, Sj[2 * p], S[2 * p], tempo[2 * p];
    uint8_t tmp[size], out[size], temp[size + 4];
    int X[2 * p * 32][500];

    InvAtlantickRoundFunction(Z, p, 0);
    sha3_4096(out, size, convert_array_32_to_8_bit(Z, 2 * p, tmp), 2 * p * 32 / 8);
    ...
    for (int s = 1; s <= round; s++)
    {
        ...
        for (int j = 0; j <= 2 * p * 32 - 1; j++)
        {
            ...
            for (int t = 0; t <= 499; t++)
            {
                ...
                sha3_4096(out, size, convert_array_32_to_8_bit(Zn, 2 * p + 1, temp), (2 * p + 1) * 32 / 8);
                convert_array_8_to_32_bit(out, size, S);

                SumBlockP(S, Sj, tempo, 2 * p);

                InvAtlantickPermutation(S, p, s);

                InvAtlantickPermutation(tempo, p, s);

                SumBlockP(S, tempo, S, 2 * p);

                X[j][t] = block_hwt(S, 2 * p);

                arith_mean += (double)(X[j][t]) / ((double)2 * p * 32 * 500);
                quad_mean += pow((double)X[j][t], 2) / ((double)2 * p * 32 * 500);
        ...
        for (int j = 0; j <= 2 * p * 32 - 1; j++)
        {
            for (int t = 0; t <= 499; t++)
            {
                std += pow((double)(X[j][t] - arith_mean), 2) / ((double)2 * p * 32 * 500);
            }
        }
        std = sqrt((double)std);
