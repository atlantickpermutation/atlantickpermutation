    ...
    for (int s = 1; s <= round; s++)
    {
        ...
        for (int j = 0; j <= 2 * p * 32 - 1; j++)
        {
            ...
            for (int t = 0; t <= 999; t++)
            {
                ...
                sha3_1024(out, size, convert_array_32_to_8_bit(Zn, 2 * p + 1, temp), (2 * p + 1) * 32 / 8);
