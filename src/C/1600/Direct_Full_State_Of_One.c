/** Full State of One Study of the Direct Atlantick Permutation
...
//////////////////////////////////////////////////////////////////////////////////////////////
//  This implementation is for q=1 (One SuperBlock), 32-bits words and a block size of 1600  //
//////////////////////////////////////////////////////////////////////////////////////////////
...
/*************************************************
* Names:        AtlantickLinearLayerOR
*
* Description: Atlantick Linear replacing ^ by |.
*
...
*         words(success)
**************************************************/
uint32_t *AtlantickLinearLayerOR(uint32_t *State, int p)
{
...
/*************************************************
* Names:        AtlantickSPboxOR
*
* Description: Atlantick SPbox replacing ^ by |.
*
* Arguments:   - uint32_t State: a block (point to an array) 
*                       of 2*p 32 bits words
*              - int p: a parameter that defines the
...
*         words(success)
**************************************************/
uint32_t *AtlantickSPboxOR(uint32_t *State, int p)
{
...
/*************************************************
* Names:        AtlantickRoundFunctionOR
*
* Description: Atlantick Permutation replacing ^ by |.
*
...
*         words(success)
**************************************************/
uint32_t *AtlantickRoundFunctionOR(uint32_t *M, int p, int r)
{
    M = AtlantickLinearLayerOR(M, p);
    M[0] = M[0] | Rc[2 * r];
    M[p] = M[p] | (uint32_t)p | Rc[2 * r + 1];
    M = AtlantickSPboxOR(M, p);
    return M;
}
...
void main()
{
    ...
    sha3_1600(out, size, convert_array_32_to_8_bit(F, 2 * p, tmp), 2 * p * 32 / 8);
    ...
    for (int j = 0; j <= 2 * p * 32 - 1; j++)
    {
        ...
        for (int t = 0; t <= 99; t++)
        {
            ...
            sha3_1600(out, size, convert_array_32_to_8_bit(Zn, 2 * p + 1, tempo), (2 * p + 1) * 32 / 8);
            convert_array_8_to_32_bit(out, size, M);

            int r = 0;
            AtlantickRoundFunctionOR(M, p, r);
